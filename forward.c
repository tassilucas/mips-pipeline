...
bool fetchStage(int qtdInstr, uint32_t *inst){
    printf("----------------------------------\n");
    printf("Fetch Stage, carregando address PC: %d\n", PC);

    if(PC < qtdInstr){
        fetDecReg.next = PC + 1;
        fetDecReg.inst = IMem[PC];
        setPC();
        return true;
    }
...
void runPipeline(int qtdInstr){
    ...
    do{
        ...
        printfileReg();
        printDMem();
    } while(fetchStage(qtdInstr, &inst));
